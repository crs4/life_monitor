...
The easiest way to manage backups of LifeMonitor data (database and workflow RO-Crates) is to use the CLI command `lm-admin.py backup`, which automatically performs a dump of the LifeMonitor database and copies all its workflow RO-Crates according to the backup settings configured on the `settings.conf` file. Such a configuration file allows to configure the local path to store the backups and, optionally, specify a remote site to sync local backups via FTP (or FTP over TLS).
